object LearningScala1 {;import org.scalaide.worksheet.runtime.library.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(127); 
  ...
  val isBest: Boolean = picard == bestCaptain;System.out.println("""isBest  : Boolean = """ + $show(isBest ));$skip(187); 
  
  // EXERCISE
  // Write some code that takes the value of pi, doubles it, and then prints it within a string with
  // three decimal places of precision to the right.
  
  val x = pi;System.out.println("""x  : Double = """ + $show(x ));$skip(14); 
  val y = x*2;System.out.println("""y  : Double = """ + $show(y ));$skip(31); 
  println(f"answer is $y%.3f");$skip(133); 
